...
func (d dashboardFactory) constructJobsForDashboard() (atc.Dashboard, error) {
	rows, err := psql.Select("j.id", "j.name", "j.human_readable", "p.id", "p.name", "p.instance_vars", "j.paused", "j.has_new_inputs", "j.tags", "tm.name",
	...
	for rows.Next() {
		...
		j := atc.DashboardJob{}
		err = rows.Scan(&j.ID, &j.Name, &j.HumanReadable, &j.PipelineID, &j.PipelineName, &pipelineInstanceVars, &j.Paused, &j.HasNewInputs, pq.Array(&j.Groups), &j.TeamName,
