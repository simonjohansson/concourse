...
var jobsQuery = psql.Select("j.id", "j.name", "j.human_readable", "j.config", "j.paused", "j.public", "j.first_logged_build_id", "j.pipeline_id", "p.name", "p.instance_vars", "p.team_id", "t.name", "j.nonce", "j.tags", "j.has_new_inputs", "j.schedule_requested", "j.max_in_flight", "j.disable_manual_trigger").
...
func (j *job) HumanReadable() string            { return j.human_readable }
...
func scanJob(j *job, row scannable) error {
	...
	err := row.Scan(&j.id, &j.name, &j.human_readable, &config, &j.paused, &j.public, &j.firstLoggedBuildID, &j.pipelineID, &j.pipelineName, &pipelineInstanceVars, &j.teamID, &j.teamName, &nonce, pq.Array(&j.tags), &j.hasNewInputs, &j.scheduleRequestedTime, &j.maxInFlight, &j.disableManualTrigger)
	if err != nil {
		return err
	}
